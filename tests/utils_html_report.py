...
def generar_html_desde_url(driver, start_url):
    """Genera un HTML interactivo con DataTables para la URL dada."""
    ...
    if not driver.find_elements(*home_page.WEB_ITEMS):
        print("No es una página válida para analizar!")
        return
    ...
    for idx, bundle in enumerate(bundles, start=1):
        try:
            ...
            if steam_page.get_url()=="https://store.steampowered.com/":
                continue
    ...
    html_tail = """
        ...
        $(document).ready(function() {
            $('#gamesTable').DataTable({ pageLength: 25, order: [[6, 'desc']] });
        });
        </script>
    </body>
    </html>
    ...
    output_path = os.path.join("reports", f"{safe_title}.html")
